...
def fetch_financial_data(tickers, start_date, end_date, synthetic_csv_path='synthetic_sp500_data.csv', cached_data_path='fetched_sp500_data.csv'):
    ...
    for ticker in tickers:
        try:
            ...
            if len(hist_prices) > 252:
                ...
                row['forward_return'] = np.random.uniform(-0.30, 0.50)
    ...
    if fetched_any and len(data_list) >= 50:
        ...
        try:
            ...
            print(
                f"Cached {len(df_fetched)} stocks to '{cached_data_path}' for future use.")
        except Exception as e:
        ...
        try:
            if os.path.exists(synthetic_csv_path):
                ...
                if 'market_cap_raw' in df_synthetic.columns:
                    df_synthetic['market_cap_log'] = np.log10(
                        df_synthetic['market_cap_raw'])
                ...
                for col in expected_cols:
                    if col not in df_synthetic.columns:
                        if col == 'ticker':
                            df_synthetic['ticker'] = [
                                f'SYN{i}' for i in range(len(df_synthetic))]
                        elif col == 'sector':
                            sectors = [
                                'Technology', 'Financials', 'Healthcare', 'Consumer Cyclical', 'Industrials']
                            df_synthetic[col] = np.random.choice(
                                sectors, len(df_synthetic))
                        elif col == 'PE_ratio':
                            df_synthetic[col] = np.random.uniform(
                                5, 30, len(df_synthetic))
                        elif col == 'DE_ratio':
                            df_synthetic[col] = np.random.uniform(
                                0.1, 3.0, len(df_synthetic))
                        elif col == 'ROE':
                            df_synthetic[col] = np.random.uniform(
                                0.05, 0.25, len(df_synthetic))
                        elif col == 'revenue_growth':
                            df_synthetic[col] = np.random.uniform(
                                -0.05, 0.20, len(df_synthetic))
                        elif col == 'profit_margin':
                            df_synthetic[col] = np.random.uniform(
                                0.01, 0.15, len(df_synthetic))
                        elif col == 'current_ratio':
                            df_synthetic[col] = np.random.uniform(
                                0.8, 3.0, len(df_synthetic))
                        elif col == 'dividend_yield':
                            df_synthetic[col] = np.random.uniform(
                                0.0, 0.05, len(df_synthetic))
                        elif col == 'beta':
                            df_synthetic[col] = np.random.uniform(
                                0.5, 1.5, len(df_synthetic))
                        elif col == 'forward_return':
                            df_synthetic[col] = np.random.uniform(
                                -0.20, 0.40, len(df_synthetic))
                        elif col == 'market_cap_log':
                            if 'market_cap' in df_synthetic.columns:
                                df_synthetic['market_cap_log'] = np.log10(
                                    df_synthetic['market_cap'])
                                df_synthetic.drop(
                                    columns=['market_cap'], inplace=True)
                            else:
                                df_synthetic[col] = np.random.uniform(
                                    8, 12, len(df_synthetic))  # Log-scale

                final_df_cols = ['ticker'] + \
                    [col for col in expected_cols if col != 'ticker']
                df_synthetic = df_synthetic[final_df_cols]
                print(
                    f"Successfully loaded synthetic data from '{synthetic_csv_path}'.")
                return df_synthetic
            else:
                raise FileNotFoundError(
                    f"Synthetic CSV '{synthetic_csv_path}' not found.")
        except FileNotFoundError:
            print(
                "Synthetic CSV not found. Generating in-memory synthetic data as fallback.")
            ...
            print(
                f"Generated {num_synthetic_stocks} synthetic stocks in memory.")
            return df_synthetic_generated
        except Exception as e:
            print(
                f"Error loading synthetic CSV: {e}. Generating in-memory synthetic data as final fallback.")
            num_synthetic_stocks = 200
            synthetic_data = {
                'ticker': [f'SYN_{i}' for i in range(num_synthetic_stocks)],
                'PE_ratio': np.random.uniform(5, 50, num_synthetic_stocks),
...
def clean_and_engineer_features(df):
    ...
    print(
        f"Dropped {initial_rows - len(df_processed)} rows with >30% missing values.")
    ...
    if 'sector' not in df_processed.columns:
        ...
        for col in numeric_cols:
            if df_processed[col].isnull().any():
                df_processed[col] = df_processed.groupby(
                    'sector')[col].transform(lambda x: x.fillna(x.median()))
        for col in numeric_cols:
            if df_processed[col].isnull().any():
                df_processed[col].fillna(
                    df_processed[col].median(), inplace=True)
    ...
    for col in numeric_cols:
        if col in df_processed.columns:
            ...
            df_processed[col] = np.clip(
                df_processed[col], lower_bound, upper_bound)

    df_processed['earnings_yield'] = df_processed['PE_ratio'].apply(
        lambda x: 1/x if x != 0 and pd.notna(x) else np.nan)
    df_processed['quality_score'] = df_processed['ROE'] * \
        df_processed['profit_margin']
    ...
    if max_de_ratio == 0 or pd.isna(max_de_ratio):
        ...
        df_processed['leverage_adj_roe'] = df_processed['ROE'] * \
            (1 - df_processed['DE_ratio'] / max_de_ratio)

    df_processed.dropna(subset=['earnings_yield', 'quality_score',
                        'leverage_adj_roe', 'forward_return'], inplace=True)

    print(
        f"Processed data shape after cleaning and engineering: {df_processed.shape}")
    return df_processed
    ...
    if q_buy == q_sell:
        ...
        def classify_return_adjusted(ret):
            if ret > q_buy:
                ...
                return 'Hold'
        df_with_target['target'] = df_with_target['forward_return'].apply(
            classify_return_adjusted)
    else:
    ...
    if len(numeric_features) > 0 and 'target' in df_final.columns:
        ...
        if num_plots_to_show > 0:
            ...
            selected_features = np.random.choice(
                numeric_features, num_plots_to_show, replace=False)
            for i, feature in enumerate(selected_features):
                plt.subplot(2, 2, i + 1)
                sns.histplot(data=df_final, x=feature, hue='target',
                             kde=True, palette='viridis', multiple='stack')
                plt.title(f'Distribution of {feature} by Target Class')
            plt.tight_layout()
            plt.show()
        else:
            print(
                "Skipping distribution plots: No numeric features available for sampling.")
    else:
        print(
            "Skipping distribution plots: No numeric features or 'target' column available.")
...
def prepare_ml_datasets(df_processed_with_signals):
    ...
    if len(np.unique(y_encoded)) < 2 or X.shape[0] < 2:
        ...
        if X.shape[0] < 2:
            print(
                "Error: Insufficient data for train/test split. Returning empty arrays.")
    ...
    df_test_metadata = df_ml.loc[X_test.index, [
...
def train_ml_model(X_train_sc, y_train, target_names, feature_cols):
    ...
    try:
        if X_train_sc.shape[0] < 5:
            print(
                "Warning: Training set too small for 5-fold CV. Training with default C=1.0.")
        else:
            grid_search = GridSearchCV(
                estimator=lr_model,
                param_grid=param_grid,
                # Use min(5, num_samples) for CV
                cv=min(5, X_train_sc.shape[0]),
                scoring='f1_weighted',
                n_jobs=-1,
                verbose=0
            )
        ...
        print(
            f"Error during GridSearchCV: {e}. Falling back to default C=1.0.")
    except Exception as e:
        print(
            f"An unexpected error occurred during GridSearchCV: {e}. Falling back to default C=1.0.")
    ...
    intercepts = pd.Series(ml_model.intercept_,
                           index=target_names, name='Intercept')
    ...
    if 'Buy' in target_names and len(feature_cols) > 0:
        ...
        buy_coefficients = coefficients.loc[target_names[buy_class_index]].sort_values(
            ascending=False)

        plt.figure(figsize=(10, 8))
        sns.barplot(x=buy_coefficients.values,
                    y=buy_coefficients.index, palette='coolwarm')
        plt.title(
            f'Logistic Regression Coefficients for "{target_names[buy_class_index]}" Class')
...
def evaluate_model_and_compare(ml_model, X_test_sc, y_test, le, target_names, df_test_metadata):
    ...
    df_test_metadata['signal_rules_encoded'] = df_test_metadata['signal_rules'].map(
        signal_map).fillna(signal_map.get('Hold', 0)).astype(int)

    # Evaluate Rules-Based Screen
    print("\n=== Rules-Based Screen Evaluation ===")
    rules_report = classification_report(
        y_test_decoded, df_test_metadata['signal_rules'], target_names=target_names, zero_division=0, output_dict=True)
    print(classification_report(y_test_decoded,
          df_test_metadata['signal_rules'], target_names=target_names, zero_division=0))

    # Evaluate ML (Logistic Regression) Classifier
    print("\n=== ML (Logistic Regression) Evaluation ===")
    ml_report = classification_report(
        y_test_decoded, y_pred_ml, target_names=target_names, zero_division=0, output_dict=True)
    print(classification_report(y_test_decoded, y_pred_ml,
    ...
    cm_rules = confusion_matrix(
        y_test_decoded, df_test_metadata['signal_rules'], labels=target_names)
    ...
    results_df['ml_buy_proba'] = y_pred_ml_proba[:, le.transform(
        ['Buy'])[0]] if 'Buy' in target_names else 0.0

    ic_ml = np.nan
    if len(results_df['ml_buy_proba'].dropna()) > 1 and len(results_df['forward_return'].dropna()) > 1:
        ic_ml, _ = spearmanr(
            results_df['ml_buy_proba'], results_df['forward_return'])
    print(f"\nInformation Coefficient (IC) for ML Model: {ic_ml:.4f}")

    buy_returns_rules = results_df[results_df['signal_rules']
                                   == 'Buy']['forward_return']
    sell_returns_rules = results_df[results_df['signal_rules']
                                    == 'Sell']['forward_return']
    spread_rules = buy_returns_rules.mean() - sell_returns_rules.mean(
    ) if not buy_returns_rules.empty and not sell_returns_rules.empty else np.nan
    print(
        f"Spread (Avg Return Buy - Avg Return Sell) for Rules-Based: {spread_rules:.4f}")

    buy_returns_ml = results_df[results_df['ml_pred_class']
                                == 'Buy']['forward_return']
    sell_returns_ml = results_df[results_df['ml_pred_class']
                                 == 'Sell']['forward_return']
    spread_ml = buy_returns_ml.mean() - sell_returns_ml.mean(
    ) if not buy_returns_ml.empty and not sell_returns_ml.empty else np.nan
    ...
    for i, class_label in enumerate(target_names):
        ...
        if len(np.unique(y_true_binary)) > 1:
            ...
            roc_auc = auc(fpr, tpr)
            ax.plot(
                fpr, tpr, label=f'ROC curve for {class_label} (area = {roc_auc:.2f})')
            valid_roc_classes.append(class_label)
        else:
            print(
                f"Skipping ROC curve for class {class_label} due to insufficient samples in test set or only one class present.")
    ...
    sns.boxplot(x='signal_rules', y='forward_return', data=results_df,
                order=target_names, palette='RdYlGn_r', ax=axes[0])
    ...
    sns.boxplot(x='ml_pred_class', y='forward_return', data=results_df,
                order=target_names, palette='RdYlGn_r', ax=axes[1])
    ...
    melted_metrics = metrics_df.melt(
        id_vars='Metric', var_name='Method', value_name='Score')
    sns.barplot(x='Metric', y='Score', hue='Method',
                data=melted_metrics, palette='viridis')
...
def main_analysis(start_date=None, end_date=None, synthetic_csv_path='synthetic_sp500_data.csv'):
    ...
    df_raw = fetch_financial_data(sp500_tickers, start_date=start_date,
                                  end_date=end_date, synthetic_csv_path=synthetic_csv_path)
    ...
    print(df_screened_rules[['ticker', 'PE_ratio', 'ROE', 'DE_ratio',
          'revenue_growth', 'profit_margin', 'signal_rules', 'target']].head())
    ...
    if X_train_sc.shape[0] == 0 or X_test_sc.shape[0] == 0:
        print(
            "Error: Insufficient data for training/testing after splits. Exiting analysis.")
        return None, None, None, None, None, None, None

    ml_model, best_C = train_ml_model(
        X_train_sc, y_train, target_names, feature_columns)

    if ml_model is None:
        print("Error: ML model training failed. Exiting analysis.")
        return None, None, None, None, None, None, None

    evaluation_metrics = evaluate_model_and_compare(
        ml_model, X_test_sc, y_test, le, target_names, df_test_metadata)
...
if __name__ == '__main__':
    ...
    ml_model, scaler, label_encoder, feature_columns, target_class_names, metrics, df_processed_final = \
        main_analysis(start_date=past_date, end_date=current_date,
                      synthetic_csv_path='synthetic_sp500_data.csv')

    if ml_model and metrics:
        print("\n--- Summary of Results ---")
        print(f"ML Model Accuracy: {metrics.get('accuracy_ml', np.nan):.4f}")
        print(
            f"Rules-Based Accuracy: {metrics.get('accuracy_rules', np.nan):.4f}")
        print(f"ML Model F1-Weighted: {metrics.get('f1_ml', np.nan):.4f}")
        print(
            f"Rules-Based F1-Weighted: {metrics.get('f1_rules', np.nan):.4f}")
        print(f"ML Model IC: {metrics.get('ic_ml', np.nan):.4f}")
        print(
            f"ML Model Spread (Buy-Sell): {metrics.get('spread_ml', np.nan):.4f}")
        print(
            f"Rules-Based Spread (Buy-Sell): {metrics.get('spread_rules', np.nan):.4f}")
        ...
        new_stock_data = pd.DataFrame([{
            ...
            # Assuming max_de_ratio was around 3 for training data
            'leverage_adj_roe': 0.18 * (1 - 0.8 / 3.0)
        }])

        df_new_stock = pd.get_dummies(new_stock_data.copy(), columns=[
                                      'sector'], drop_first=True)
        ...
        print(
            f"Prediction probabilities: {dict(zip(target_class_names, new_pred_proba[0]))}")
    else:
